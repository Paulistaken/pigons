...
pub mod dane_out {
    ...
    impl BusEvent {
        pub fn export_tdf_plan(&self) -> (String, String) {
            let mut uprow = "".to_string();
            let mut downrow = "".to_string();

            uprow += "bus id";
            downrow += &format!("{},", self.id_of_the_bus.id_number);
            for step in self.previous_stations.iter() {
                let timeexp = step.time_of_event.export_tdf();
                uprow += &format!("previous station id,{},", timeexp.0);
                downrow += &format!("{},{},", step.id_of_the_station.id_number, timeexp.1);
            }
            {
                let timeexp = self.time_of_event.export_tdf();
                uprow += &format!("current station id,{},", timeexp.0);
                downrow += &format!("{},{},", self.id_of_the_station.id_number, timeexp.1);
            }
            for step in self.next_stations.iter() {
                let timeexp = step.time_of_event.export_tdf();
                uprow += &format!("future station id,{},", timeexp.0);
                downrow += &format!("{},{},", step.id_of_the_station.id_number, timeexp.1);
            }
            (uprow, downrow)
        }
